...
New paper self, guardian, and dependent records run through `DuplicateDetectionService` before the application write. Exact email or phone collisions hard-block new-record creation so paper intake cannot silently attach an unrelated user. Soft name+DOB/address matches open `DuplicateReviewCase` rows through `DuplicateReviewCases::CreateService` after the subject user is persisted; paper-created cases use source `:paper_intake`, set `users.needs_duplicate_review`, and log `duplicate_review_case_opened`.
