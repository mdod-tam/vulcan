...
Flag/case sync is enforced in both directions: every outcome or merge recomputes the subject/canonical `needs_duplicate_review` flag from all pending states, a merge resolves the actionable pair-related pending cases that would otherwise be stranded on the retired duplicate, and the legacy `clear_flag` path refuses to clear a flag while any pending case remains. Case creation locks the subject and candidates before inserting a pending case or setting the flag, rejecting a merged, inactive, or suspended subject or a merged candidate so it cannot recreate review work on an identity retired by a concurrent merge; an unmerged suspended/inactive candidate remains reviewable. The shared semantic deduplication key covers source, subject, reasons, and candidates; a unique-index race returns the committed pending case as idempotent success. Awaiting-information or security-review cases block merging—including through a related case—until staff explicitly return them to normal review.

### 3.2.2 Same-person merge service

`Users::DuplicateMergeService` performs a same-person merge of one constituent record into a canonical survivor. It requires an admin actor, an actionable (`open`) review case, explicit same-person confirmation, a rationale, evidence/reason codes, and explicit contact and delivery choices. It locks both users before dependent records, then locks the related pending cases, preflights its declared blocker inventory, performs all mutations with bang persistence inside one transaction, rolls back on failure, and emits exactly one `duplicate_user_merged` audit event. Identity-sensitive writers use that same user-first lock order and revalidate path-specific eligibility while holding the lock: portal final submission/autosave, selected-participant paper intake, constituent/guardian-dependent/admin contact edits, duplicate-case creation, password reset consumption, recovery creation/approval, and bearer-form issuance therefore serialize with retirement instead of committing stale work afterward. User rows lock through the base `User` relation in ascending ID order so legacy STI rows participate in the same contract.

A merged duplicate is deactivated (`status: inactive`) and points at its survivor through `users.merged_into_user_id`, with `merged_by_id` and `merged_at` recorded. It is never destroyed. `User#public_login_active?` rejects merged, inactive, and suspended records (treating legacy NULL status as active). It gates the auth-lookup helper (`find_by_login_identifier`) and session-cookie creation (`_create_and_set_session_cookie`), which is the single chokepoint for both password sign-in and 2FA completion, so a duplicate retired mid-login cannot finish authenticating. Password-reset consumption separately re-resolves the generated token and rechecks account activity while holding the user lock; an email-changing merge therefore invalidates an already-resolved request before it can update the password.

Merge inventory (decision per area):

| Area | Decision |
|------|----------|
| Applications | Transfer all duplicate-owned applications to the canonical user by FK repoint, preserving status, history, and audit. Portal final submission and autosave plus existing-participant paper intake lock and revalidate participating users before the application: a writer that wins is visible to merge inventory, while merge that wins makes the stale writer fail. Final portal submission has one routed path (`ApplicationsController#update` → `ApplicationCreator`); the unused direct member `submit` route is removed. If the canonical was the managing guardian of a transferred application, the managing guardian is cleared first so the merged record is never self-managed. Blocked if it would leave the canonical record with more than one active/blocking application. |
...
| Password reset / recovery state | Legacy `reset_password_token` and `reset_password_sent_at` columns are cleared on retirement. Generated Rails password-reset tokens are tied to password salt + normalized email, and token consumption revalidates both token and account activity under the user lock. Merge is blocked if the duplicate has a pending recovery request; recovery creation and approval also lock and recheck the user, while resolved requests remain retired-record history. |
| Secure request forms | Blocked if the duplicate is the recipient of an active bearer-link form. Issuance locks and validates possible recipient users before the application: issuance that wins creates a blocker visible to merge, while merge that wins makes issuance reject the retired recipient. |
| Contact facts | Admin explicitly chooses the surviving email, phone, phone type, and address; synthetic or effective fallback values never become stored record truth. After those values are captured, the retired duplicate releases its primary email and phone so global identity lookup, duplicate detection, and unique indexes do not keep discarded contact live. The PR193 data migration applies the same release to rows merged under PR192's source-only cleanup. Constituent, guardian-dependent, paper-intake, and admin mutation boundaries lock/recheck the user, and `UserMergeIntegrity` rejects any later attempt to give a merged row primary email or phone. A real surviving phone requires an explicit phone type. A missing or invalid per-field choice blocks the merge rather than silently defaulting to canonical. |
...
| Duplicate review cases | The selected case and actionable pair-related pending cases resolve to `resolved_merged`; candidate snapshots and evidence are retained. A related awaiting-information or security-review case blocks merge until staff return it to normal review. Pending cases about an unrelated subject are requalified after their case-row locks: a case resolved while merge waited is terminal history and is not changed. Still-pending candidate links repoint/deduplicate to the survivor, and their semantic deduplication keys are recomputed in the same transaction; a projected collision with another pending case blocks merge. |
