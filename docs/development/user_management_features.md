...
| Portal dependent creation | `app/controllers/constituent_portal/dependents_controller.rb` | Blocks exact contact collisions before persistence, then locks and requalifies the signed-in guardian plus any soft-match review candidates. The new dependent, guardian relationship, and — for a soft match — the duplicate-review case with its candidate rows, subject review flag, and case-opened audit event all commit atomically, with no compensating delete; guardian contact snapshots come from the locked guardian. A failed attempt re-renders the submitted contact choice rather than the internal placeholder values it generated. |
