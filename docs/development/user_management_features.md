...
| Duplicate flag projection | `app/services/duplicate_reconciliation/review_flag_projection.rb`, `app/services/duplicate_reconciliation/review_flag_sync_service.rb` | Defines the review flag as participation in any open case of any source or an unresolved post-import pair. `duplicates:sync_review_flags` creates no cases, events, or notifications and reports before/after/set/clear counts. |
...
| Duplicate review workflow | `app/controllers/admin/duplicate_reviews_controller.rb`, `app/views/admin/duplicate_reviews/` | Existing queue and detail workflow, extended with unresolved post-import pairs. The queue groups connected pair actions for scanning; groups of three or more offer the first stable pair as `Compare next` and progressively disclose every pair choice, but each action still opens exactly one pair-scoped case. The durable outcomes remain the non-merge resolution and same-person merge. |
| Duplicate resolution service | `app/services/duplicate_review_cases/resolution_service.rb` | Records a non-merge resolution. Both the *status* (`resolved_ignored`, displayed as "Resolved without merge") and the *determination* (`keep_separate`) are server-owned; the service takes neither as a parameter. The admin supplies the required different-person rationale, while the controller supplies the bounded `admin_reviewed` audit code instead of asking staff to restate the case-opening match evidence. It reprojects every constituent participant's flag; post-import cases must remain exact current pairs under lock. |
...
- Soft matches create `DuplicateReviewCase` rows through `DuplicateReviewCases::CreateService` only on flows that still defer the decision. Public registration uses source `:registration_soft_match`. Paper self, guardian quick-create, and dependent intake adjudicate inline: staff select an eligible existing record or explicitly confirm that surfaced candidates are different people, and no `paper_intake` or `admin_create` case is opened.
- `users.needs_duplicate_review` is a service-owned projection, not a controller-helper or model-callback decision. Case creation sets its subject signal; post-import synchronization, resolution, and merge reproject every participant they own from durable open cases and unresolved pairs.
- Portal dependent creation uses context `:portal_new_dependent` and source `:portal_dependent`.
- Paper guardian quick-create and paper self/dependent creation use contexts `:paper_new_guardian`, `:paper_new_self`, and `:paper_new_dependent`. These adjudicated writers preserve the historical `admin_create` and `paper_intake` enum values and rows but create no new case in those sources.
...
- The durable pair shape is strict: lower user ID as `subject_user`, higher user ID as the sole candidate, candidate `match_reason: name_dob`, and case `reason_codes: [name_dob]`. Reversed requests canonicalize to that shape. Multi-candidate, reversed-orientation, or otherwise malformed cases are never accepted as pair-level suppression evidence.
...
- `bin/rails duplicates:sync_review_flags` projects the Boolean cache from durable state. A non-retired constituent requires the flag when they are subject or candidate in an open case of any source, or when they participate in an unresolved post-import pair. It creates no case/audit/notification, preserves every source, clears only when neither authority remains, reports counts, and is idempotent.
...
|------|---------|-------------------------|
| `bin/rails duplicates:discovery` | Production-safe read-only architectural baseline probe evaluating case counts, strict post-import shapes, malformed cases, true review-flag drift, dynamic match clusters, and multi-guardian dependent distributions. | Runs in a database-enforced `SET TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY` transaction (ensuring a consistent snapshot across all queries). Fails closed if invoked within an open transaction. Dynamically sets PostgreSQL `statement_timeout` via `set_config` before application queries to the remaining time in the 15-second task budget (with transaction setup and `set_config` itself preceding each timed query), actively enforcing the total deadline in the database. All queries execute as database aggregates (`COUNT`, `GROUP BY`, `HAVING`, CTEs) with `LIMIT` on sample queries. Detailed mode bounds relationship samples per dependent and reports omitted counts. Default mode redacts all PII, omitting constituent names from rendered output and bounding sample queries (the largest-cluster query reads normalized names/encrypted DOB only within the service to compute an opaque cluster token), masks database topology using an application-secret HMAC fingerprint, and generates reference tokens using an HMAC keyed with a cryptographically secure per-run random salt to prevent identifier enumeration. |
| `DETAILED_PII=true bin/rails duplicates:discovery` | Explicit opt-in diagnostic mode that includes dependent and guardian names for multi-guardian sample dependents and raw record identifiers. | **SENSITIVE OUTPUT:** Emits real constituent names and relationships. Output must never be committed to source control or logged in public/unrestricted terminal files. In production, operators must record execution using `.cursor/bin/record-check --sensitive` without saving stdout or redirecting raw PII to persistent files under `.agent-work/evidence` or logs. |
