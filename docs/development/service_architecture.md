...
| `Applications::PaperIdentityCreationLock` | `app/services/applications/paper_identity_creation_lock.rb` | Sole owner of the transaction-scoped Postgres name/date-of-birth advisory lock shared by every retained paper new-person writer. Competing writers wait for the same identity key; database lock failures raise. | Returns the block result after acquiring the lock |
