...
| `Users::DuplicateMergeService` | `app/services/users/duplicate_merge_service.rb` | Same-person merge of a duplicate constituent into a canonical survivor: locks both records + related pending cases, blocks held cases, preflights every blocker, transfers applications/guardian relationships, retires the duplicate, resolves actionable related cases, and emits one `duplicate_user_merged` event, all in one transaction. | `BaseService::Result` |
