...
| `Applications::PaperApplicationService` | `app/services/applications/paper_application_service.rb` | Creates admin-entered paper applications through the `new`/`create` admin intake route. Existing selected applicants/guardians/dependents lock and revalidate before application creation; updates lock current owners before the application. | Boolean plus `errors` / `reconciliation_note` |
| `Applications::ApplicationCreator` | `app/services/applications/application_creator.rb` | Creates and performs final constituent-portal submission after locking and revalidating every participating user, then locking any existing application, so a merge cannot retire the applicant between authentication and persistence. | `Applications::ApplicationCreator::Result` |
| `Applications::AutosaveService` | `app/services/applications/autosave_service.rb` | Saves permitted portal draft fields under the same ordered user-before-application lock contract as final submission; merge-first requests reject retired participants and writer-first drafts remain visible to merge inventory. | Hash |
...
| `Applications::RequestProviderInfo` / `Applications::RequestProofResubmission` | `app/services/applications/request_provider_info.rb`, `app/services/applications/request_proof_resubmission.rb` | Issue bearer-link request forms only after locking and revalidating the possible recipient users ahead of the application, preventing delivery to a record retired by a concurrent merge. | `BaseService::Result` |
...
| `DuplicateReviewCases::CreateService` | `app/services/duplicate_review_cases/create_service.rb` | Locks the subject and candidate users before idempotently opening a duplicate-review case, rejects a merged, inactive, or suspended subject or a merged candidate, stores sanitized candidate/metadata snapshots, syncs `needs_duplicate_review`, and logs the case-opened audit event. It uses the shared semantic key builder and treats a pending-case unique-index race as idempotent success. Suspended or otherwise inactive unmerged candidates remain reviewable. | `BaseService::Result` |
| `DuplicateReviewCases::ResolutionService` | `app/services/duplicate_review_cases/resolution_service.rb` | Records one outcome from an actionable case: keep-separate or persisted-relationship outcomes resolve terminally; awaiting-information/security-review outcomes remain pending. Requires admin+rationale, moves no user-owned data, recomputes the flag from all pending states, and logs the outcome transactionally. | `BaseService::Result` |
| `DuplicateReviewCases::ResumeService` | `app/services/duplicate_review_cases/resume_service.rb` | Returns an awaiting-information or security-review case to normal actionable review with an admin actor, rationale, timestamp, flag sync, and dedicated audit event in one transaction. | `BaseService::Result` |
| `Users::DuplicateMergeService` | `app/services/users/duplicate_merge_service.rb` | Same-person merge of a duplicate constituent into a canonical survivor: locks both users before dependent records, blocks held cases, preflights its declared blocker inventory, transfers applications/guardian relationships, reconciles only still-pending third-party candidate links, retires the duplicate, resolves actionable pair-related cases, and emits one `duplicate_user_merged` event, all in one transaction. Candidate repoint/deduplication and semantic-key recomputation are atomic; projected pending-key collisions block merge. Concurrent identity-sensitive writers use the same user-first lock order and revalidate path-specific eligibility under that lock. | `BaseService::Result` |
...
Merge-sensitive user locks use ordered `User` base-class queries rather than subclass `with_lock`. Besides preventing deadlocks, this preserves compatibility with legacy STI rows whose stored `type` predates the current namespaced class name. Authentication may have memoized an active user earlier in a request, so persistence boundaries—not the cached object—own the authoritative post-lock retirement check.
