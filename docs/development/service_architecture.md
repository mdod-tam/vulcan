...
| `Applications::ApplicationCreator` | `app/services/applications/application_creator.rb` | Creates constituent-portal applications after locking and revalidating every participating user, then locking any existing application, so a merge cannot retire the applicant between authentication and persistence. | `Applications::ApplicationCreator::Result` |
...
| `Users::DuplicateMergeService` | `app/services/users/duplicate_merge_service.rb` | Same-person merge of a duplicate constituent into a canonical survivor: locks both users before dependent records, blocks held cases, preflights its declared blocker inventory, transfers applications/guardian relationships, reconciles pending third-party case candidate links, retires the duplicate, resolves actionable pair-related cases, and emits one `duplicate_user_merged` event, all in one transaction. Concurrent identity-sensitive writers use the same user-first lock order and revalidate path-specific eligibility under that lock. | `BaseService::Result` |
