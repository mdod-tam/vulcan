...
| `Applications::PaperApplicationEligibility` | `app/services/applications/paper_application_eligibility.rb` | Sole paper owner of the blocking-application and waiting-period admission rule for an existing constituent. Paper review asks it when marking applicant/dependent candidates selectable; `PaperApplicationService` asks again at the durable writer boundary and owns the refusal result. It does not replace portal sibling-application eligibility. | `PaperApplicationEligibility::Result` |
| `Applications::PaperIdentityReview` | `app/services/applications/paper_identity_review.rb` | **Sole owner** of paper self-applicant, guardian, and dependent review facts, candidate presentation/selectability composition, outcomes, and signed issuance/verification. Dependent facts deterministically apply guardian/dependent contact strategies and bind guardian plus relationship context. A dependent row is selectable only when `PaperApplicationEligibility` admits it and an existing `GuardianRelationship` links it to the selected guardian. Every paper writer consumes the same owner. | `PaperIdentityReview::Result` (keyword-init struct) |
...
| `Applications::PaperGuardianQuickCreateService` | `app/services/applications/paper_guardian_quick_create_service.rb` | Canonical JSON paper guardian writer. Recomputes review under the shared identity lock, requalifies selected candidates under row lock, creates with `skip_user_lookup`, commits a resolved paper case for an explicit selection or keep-separate decision. A unique-contact race rolls back, then is reclassified through a fresh identity review so staff receive a block or retry instruction rather than database constraint text. | `BaseService::Result` with state, review, user, and created flag |
| `Applications::PaperIdentityReviewReceipt` | `app/services/applications/paper_identity_review_receipt.rb` | Rails-verifier-backed, purpose-scoped receipt of keyed identity and candidate digests. Binds actor, role/context, displayed facts and selectability; expires after 30 minutes. Every writer recomputes under identity and participant locks. Completed decisions belong to `DuplicateReviewCase`, with scoped replay uniqueness. | `PaperIdentityReviewReceipt::Result` |
| `Applications::GuardianDependentManagementService` | `app/services/applications/guardian_dependent_management_service.rb` | Creates a new paper dependent for a locked, eligible selected guardian; canonically writes the guardian/dependent email, phone, and address strategy shapes interpreted by `UserGuardianship`; verifies identity under the shared lock; returns the verified review to the outer application writer for atomic case persistence; and creates the relationship in the outer paper transaction. It does not create guardians or reuse unrelated existing dependents. | `BaseService::Result` on the success path |
...
| `Applications::RequestProviderInfo` | `app/services/applications/request_provider_info.rb` | Canonical writer of provider-info `SecureRequestForm` rows. Issues and resends secure provider-contact links under the issuance-integrity lock, routing each recipient through `Applications::SecureRequestRecipientResolver` (logical recipient, model-owned contact/address ownership, deliverable channel, delivery-owner eligibility), persisting both delivery owner and source, and delivering via `ApplicationNotificationsMailer`, `SmsService`, or `PrintQueueItem`. | `BaseService::Result` |
| `Applications::RequestProofResubmission` | `app/services/applications/request_proof_resubmission.rb` | Canonical writer of proof-resubmission `SecureRequestForm` rows. Same resolver-routed issuance under the issuance-integrity lock for rejected/requested proofs, including complete delivery provenance on every new row. | `BaseService::Result` |
...
| `DuplicateReviewCases::CreateService` | `app/services/duplicate_review_cases/create_service.rb` | Opens idempotent deferred cases with sanitized candidate/metadata snapshots, subject review flags, and case-opened audit events. `record_paper_decision!` composes case creation and resolution inside the intake transaction: keep-separate records one resolved case per new-person/candidate pair; existing selection records a proposed-identity fingerprint and selected candidate without a second user. Clear creation requires no case. | `#call`: `BaseService::Result`; `.record_paper_decision!`: array of cases; raises if recording or resolution fails |
| `DuplicateReconciliation::Population` / `PairGroup` / `Report` | `app/services/duplicate_reconciliation/population.rb`, `app/services/duplicate_reconciliation/pair_group.rb`, `app/services/duplicate_reconciliation/report.rb` | Read-only owner of eligible normalized-name/encrypted-DOB groups, stable unordered pairs, derived pair state, and privacy-bounded text/CSV report output. Pair-evidence qualification belongs to `DuplicateReviewCase.reconciliation_pairs`, which includes strict post-import cases and terminal inline paper keep-separate pairs; Population applies state precedence. `PairGroup` is a queue-only presentation projection over those pairs and creates no group-level decision or suppression state. Retired identities appear only as historical `merged_retired` context, never unresolved work. | Pair/Member data objects, grouped pair projection, and `Report::Result` |
...
| `DuplicateReconciliation::ReviewFlagProjection` / `ReviewFlagSyncService` | `app/services/duplicate_reconciliation/review_flag_projection.rb`, `app/services/duplicate_reconciliation/review_flag_sync_service.rb` | Defines `needs_duplicate_review` from any-source open-case participation plus unresolved current name+DOB pairs. Population applies recognized post-import and inline paper decisions before identifying unresolved work. The sync service locks one non-retired constituent at a time, rebuilds current projection state under that lock, creates no cases/events/notifications, and reports before/after/set/clear counts. | Boolean projection and `BaseService::Result` counts |
| `DuplicateReconciliation::DiscoveryProbe` | `app/services/duplicate_reconciliation/discovery_probe.rb` | Production-safe read-only architectural baseline probe evaluating duplicate review case counts, strict vs malformed post-import shapes, true review-flag drift, dynamic matching clusters (aligned with canonical `Population`), and multi-guardian dependent distributions. Runs within a `REPEATABLE READ READ ONLY` transaction with per-query statement timeouts bounded by a task deadline. Resolves release provenance (`code_sha`) from deployment release environment variables (`COMMIT_SHA`, `HEROKU_BUILD_COMMIT`, `KAMAL_VERSION`, etc.) with git fallback. Evaluates logical date decryptability matching canonical `Population` using database aggregates and bounded audits without unbounded in-memory loading (separating raw SQL totals from verified canonical metrics, reporting explicit truncation flags and lower bounds when bounded audits are incomplete, and never logging raw stored dates). Detailed mode safely encodes control, format, and separator characters (`Cc`, `Cf`, `Zl`, `Zp`) at the output boundary to prevent terminal manipulation, bidi reordering, or report spoofing. Returns a structured `Result` with PII, raw metadata, and raw identifiers redacted by default, masking topology with an application-secret HMAC fingerprint and record identifiers with per-run HMAC references. | `DiscoveryProbe::Result` |
| `DuplicateReviewCases::ResolutionService` | `app/services/duplicate_review_cases/resolution_service.rb` | Resolves an open case without moving data. Admin `#call` records `keep_separate` / `resolved_ignored`; inline paper `#select_existing` validates the selected candidate and records `existing_person_selected` / `resolved_selected`. Both outcomes are server-owned. It locks actor plus known case participants, then the case and candidate rows; refuses a changed participant set; and reprojects each locked constituent participant. A `post_import_reconciliation` case additionally must retain the model-owned strict pair shape and current supported match under lock. The registration submission gate remains separately subject/source-scoped. | `BaseService::Result` |
...
PR4c (#209) owns resolver selection, issuance serialization, and persisted delivery
provenance. The PR4b stack (#192–#194) owns same-person merge behavior and its
secure-request blockers. The two issuance services above are the only production writers
of application `SecureRequestForm` rows. New rows require both `delivery_owner` and
`delivery_source`; nullable columns preserve truthful historical rows, whose missing
...
- merge: active actor, canonical, duplicate, relationship neighbors, and related-case participants → selected plus related open cases and candidate evidence → all applications owned or managed by either participant → every guardian relationship touching either participant. Before mutation, the locked preflight also checks active secure requests against the captured final contact decision. Another exact-pair post-import case containing the retiring record is projected onto the survivor under these locks; malformed and other-source cases containing the retiring record still fail closed;
