...
| `Applications::PaperIdentityCreationLock` | `app/services/applications/paper_identity_creation_lock.rb` | Sole owner of the transaction-scoped Postgres name/date-of-birth advisory lock shared by every retained paper new-person writer. | Raises on lock failure; otherwise returns adapter result |
