...
| `Applications::PaperApplicationService` | `app/services/applications/paper_application_service.rb` | Creates admin-entered paper applications through the `new`/`create` admin intake route. **Result contract:** `false` means nothing committed and the caller must re-render the form with `errors`; `true` means the application committed, with `warning_message` present when a post-commit step failed. Commit state is established by querying the database, never by `application.persisted?` -- a rollback clears the id, and an `after_commit` callback can raise after the data is durable (`ProofReview` does, on every rejected proof). On the new self-applicant branch it takes a transaction-scoped Postgres advisory lock keyed on the canonical name + date of birth (the same equivalence `Users::Constituent.find_duplicates` uses), then delegates the whole identity question to `PaperIdentityReview` and consumes its answer without re-deriving any part of it. The lock is what makes two concurrent submissions of the same person produce exactly one record; the unique contact index cannot see the case where the two submissions carry different (or no) email and phone. | Boolean plus `errors` / `warning_message` |
| `Applications::PaperIdentityReview` | `app/services/applications/paper_identity_review.rb` | **Sole owner** of what a search for a paper self-applicant turns up and what staff may do about it. Read-only. Runs `DuplicateDetectionService` in the `paper_new_self` context, applies `PaperContactFlags` first (so no-email/no-phone change the facts before detection), classifies the result as `clear` / `needs_confirmation` / `blocked` / `confirmed` / `error`, decides which candidates are `selectable`, builds the **presented snapshot** the browser renders, and issues or verifies the decision token. Both callers — the `identity_review` preview endpoint and `PaperApplicationService` at the write boundary — go through this one object so a preview and a write can never disagree. Applies to the **new self-applicant** branch only. | `PaperIdentityReview::Result` (keyword-init struct) |
| `Applications::PaperIdentityDecision` | `app/services/applications/paper_identity_decision.rb` | Issues and verifies the signed attestation that staff looked at specific candidates and decided none is this applicant. Token is `v1:<issued_at>:<hmac>`; nothing about the applicant travels in it. Bound to the context, admin, an HMAC of the full detection fact set, an HMAC of the **presented candidate snapshot including each row's `selectable` state**, the reason codes, and the issue time. 30-minute expiry. Payload is recomputed and re-signed at the write boundary, so any bound fact that changed invalidates it. Stateless — never marked spent — so replay is excluded by the identity advisory lock in `PaperApplicationService`, not here. | `PaperIdentityDecision::Result` |
