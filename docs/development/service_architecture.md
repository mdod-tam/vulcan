...
| `DuplicateReconciliation::DiscoveryProbe` | `app/services/duplicate_reconciliation/discovery_probe.rb` | Production-safe read-only architectural baseline probe evaluating duplicate review case counts, strict vs malformed post-import shapes, true review-flag drift, dynamic matching clusters (aligned with canonical `Population`), and multi-guardian dependent distributions. Runs within a `REPEATABLE READ READ ONLY` transaction with per-query statement timeouts bounded by a task deadline. Evaluates logical date decryptability matching canonical `Population` using database aggregates and bounded audits without unbounded in-memory loading (separating raw SQL totals from verified canonical metrics, reporting explicit truncation flags and lower bounds when bounded audits are incomplete, and never logging raw stored dates). Returns a structured `Result` with PII, raw metadata, and raw identifiers redacted by default, masking topology with an application-secret HMAC fingerprint and record identifiers with per-run HMAC references. | `DiscoveryProbe::Result` |
