...
| `DuplicateDetectionService` | `app/services/duplicate_detection_service.rb` | Evaluates exact contact duplicates and soft name+DOB/address signals for public registration, admin quick-create, and paper self/guardian/dependent creation contexts. | `BaseService::Result` with `DuplicateDetectionService::Result` data |
