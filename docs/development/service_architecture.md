...
| `DuplicateReviewCases::ResolutionService` | `app/services/duplicate_review_cases/resolution_service.rb` | Resolves an open case without moving data, locking actor + subject before the case, syncing the subject `needs_duplicate_review` flag, and logging `duplicate_review_case_resolved`. The determination is **server-owned**: it always records `NON_MERGE_DETERMINATION` (`keep_separate`) and takes no determination parameter, because only a completed identity decision may close a case. Resolving recomputes two independent effects from remaining open cases — the submission gate releases when no open `registration_soft_match` case remains, and the review flag clears when no open case of any source remains. `same_person_confirmed` is reserved to the merge service. | `BaseService::Result` |
