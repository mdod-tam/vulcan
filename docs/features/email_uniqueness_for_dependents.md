...
- `has_own_contact_info?` and `uses_guardian_contact_info?` are still not implemented helper methods.
- Paper intake already exposes contact strategy selection for dependent contact handling.
- Broader dependent contact management outside the paper flow should reuse the same effective-contact methods and encrypted search-token approach.
