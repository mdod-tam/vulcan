...
| Paper identity decision | `paper_identity_no_match_confirmed` | The canonical writer for each context (`Applications::PaperApplicationService`, `Applications::PaperGuardianQuickCreateService`, or `Applications::GuardianDependentManagementService`) emits exactly one per successful override, attached to the newly created constituent with the admin as actor. Written **only** when staff overrode surfaced candidates; clear creation and existing-record selection log nothing. Every row carries `candidate_ids`, `candidate_count`, and `reason_codes`. Guardian quick-create additionally carries `decision_context: guardian_quick_create` and `role: guardian`; new-dependent creation carries `decision_context: paper_new_dependent`, `role: dependent`, `guardian_id`, and `relationship_type`. Self-applicant rows carry no context or role key. No row stores raw identity facts or the decision token. This bounded audit evidence is not pair-addressable duplicate-reconciliation state and does not authorize a merge or suppress later population review. |
...
Duplicate-review case audit actor selection is flow-specific: public registration uses `PublicAuditActor` and rolls back account creation if no system actor can open the required case; portal dependent creation uses the signed-in guardian; post-import pair entry uses the active administrator who started the exact pair comparison. Historical `admin_create` and `paper_intake` cases retain their original current-admin/operator attribution. Post-import case metadata and events contain IDs, source, and bounded reason codes, never raw email, phone, or DOB values. The detail UI presents case-opening match reasons as read-only context. A keep-separate resolution records the admin's required rationale with the server-owned bounded code `admin_reviewed`; a same-person merge carries the case's bounded match reasons plus the admin's required explanation, so neither form asks staff to select or restate audit codes.

Paper **self-applicant, guardian quick-create, and dependent** intake no longer opens `paper_intake` or `admin_create` duplicate-review cases. Staff resolve the question inline before the write, so a case would queue a decision that had just been made. `paper_identity_no_match_confirmed` is written only for a successful different-person override and records actor, candidate ids/count, and reason codes without raw facts or tokens. Guardian and dependent rows also record their context/role metadata described above. Clear creation and existing-record selection write no identity-decision event.
