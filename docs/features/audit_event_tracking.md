...
| Duplicate review | `duplicate_review_case_opened`, `duplicate_review_case_resolved`, `duplicate_user_merged`, `duplicate_review_flag_cleared` | `DuplicateReviewCases::CreateService` owns case-opened rows; `DuplicateReviewCases::ResolutionService` owns approve/ignore/keep-separate resolutions; `Users::DuplicateMergeService` emits exactly one `duplicate_user_merged` per merge; `DuplicateReviewCases::ClearFlagService` (invoked by `Admin::DuplicateReviewsController#clear_flag`) locks the subject row, requalifies it, and logs legacy-flag clears. |
