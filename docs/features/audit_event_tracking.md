...
Duplicate-review case audit actor selection is flow-specific: public registration uses `PublicAuditActor` and rolls back account creation if no system actor can open the required case; portal dependent creation uses the signed-in guardian. Historical `admin_create` and `paper_intake` cases retain their original current-admin/operator attribution.
