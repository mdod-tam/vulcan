...
- [ ] **Retire the `approve`/`ignore` actions and their UI together (5c-2).** Remove the action fieldset, stop accepting `resolution_action` from the request, update the surrounding copy, and preserve historical values. These must land as one change: backend-first leaves Approve and Ignore visible while the server rejects them; UI-first changes which action values are newly written, which is what the external check gates. Three constraints on the implementation:
  - **Only derive compatible metadata server-side if the consumer inventory finds a consumer that needs it.** If the inventory is clean, stop writing `resolution_action` outright rather than emitting a derived value nobody reads. Any compatibility writer must carry an explicit removal condition, or it becomes permanent by default.
...
- [ ] **Blocks 5c-2:** verify — do not assume — whether any external consumer reads `status` or `resolution_action`. The in-repo half is **swept and clean**: `resolution_action` is write-only, with no reader in reports, exports, rake tasks, JSON payloads, audit-log display, webhooks, or database views. The open question is entirely out-of-repo — BI exports, warehouse queries, scheduled reports, and manual operational SQL. Note that having no production data does **not** answer it; reporting can be built before meaningful data exists. Ask each owner whether anything they own reads the field at all, expects it present on every resolution event, groups or filters on its values, infers meaning from `resolved_approved` versus `resolved_ignored`, or lives in manual SQL outside the repository.
...
- [ ] Decide the guardian-authority policy. The submission gate checks only the applicant, never the acting guardian, so a guardian with their own open case can submit for a dependent. Current behavior is deliberate and test-pinned; ratifying it costs one documentation sentence, while changing it is a further implementation slice.
