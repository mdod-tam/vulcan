...
require_relative 'email_template_migration_helpers'

class UpdateTextEmailLinkAccessibilityCopy < ActiveRecord::Migration[8.1]
  include EmailTemplateMigrationHelpers
  ...
  # Align stored variable metadata with seed files when body copy is unchanged.
  VARIABLE_ONLY_UPDATES = [
    {
    ...
    },
    {
      ...
      locale: 'en',
      variables: {
        'required' => %w[constituent_full_name request_count_message constituent_dob_formatted constituent_phone_formatted
                         constituent_email constituent_address_formatted application_id certification_submission_instructions
                         support_email],
        'optional' => []
      }
    },
  ...
  def up
    ...
    VARIABLE_ONLY_UPDATES.each do |attributes|
      sync_email_template_variables!(
  ...
  def remove_staff_only_translation_templates!
    delete_email_template_records!(
      Record.where(name: STAFF_ONLY_TEMPLATE_NAMES, format: TEXT_EMAIL_FORMAT, locale: 'es')
    )
  end

  def update_template!(attributes)
    upsert_email_template_record!(
      name: attributes.fetch(:name),
      locale: attributes.fetch(:locale),
      attributes: attributes.slice(:subject, :description, :body, :variables)
    )
  end
end
