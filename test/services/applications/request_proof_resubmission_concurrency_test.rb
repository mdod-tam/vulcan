...
module Applications
  ...
  class RequestProofResubmissionConcurrencyTest < ActiveSupport::TestCase
    ...
    test 'merge commits first: dependent proof issuance re-resolves the retired guardian delivery owner' do
      admin, canonical, duplicate, review_case = build_fixtures
      ...
      contender_thread = on_own_connection do
        contender_pid_queue << backend_pid
        issuance_result = run_proof_resubmission_request(admin:, application:, recipient: dependent)
      end
    ...
    test 'dependent proof issuance commits first: merge blocks on the retiring guardian delivery owner' do
      admin, canonical, duplicate, review_case = build_fixtures
      dependent = create(:constituent)
      create(:guardian_relationship, guardian_user: duplicate, dependent_user: dependent)
      application = create(:application, user: dependent, managing_guardian: duplicate)
    ...
    def run_proof_resubmission_request(admin:, application:, recipient: nil)
      args = {
      ...
      RequestProofResubmission.new(**args).call
    end
  end
end
