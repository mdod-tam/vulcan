...
module Applications
  ...
  class ApplicationCreatorConcurrencyTest < ActiveSupport::TestCase
    ...
    test 'submission commits first: the merge then proceeds normally, transferring the now-submitted application' do
      admin, canonical, duplicate, review_case = build_fixtures
      draft = create(:application, :draft, user: duplicate)
      ...
      assert_equal canonical.id, draft.reload.user_id, 'the submitted application must have transferred to the canonical survivor'
    ensure
      cleanup_duplicate_review_test_data!(admin, canonical, duplicate)
    end
