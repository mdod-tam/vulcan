...
class EmailTemplateSnapshotTest < ActiveSupport::TestCase
  ...
  test 'version still increments on subject or body change' do
    original_version = @template.version

    @template.update!(subject: 'Updated subject', body: 'Updated body %<name>s')

    assert_equal original_version + 1, @template.version
  end

  test 'record! skips insert when before_attributes show no render-relevant change' do
    assert_no_difference -> { @template.email_template_snapshots.count } do
      result = EmailTemplateSnapshot.record!(
        template: @template,
        change_source: 'admin_edit',
        actor: @admin,
        before_attributes: @before_attributes
  ...
  test 'render_relevant_change_between? accepts string-keyed before attributes' do
    string_keyed = @before_attributes.stringify_keys

    @template.update!(subject: 'Changed')

    assert EmailTemplateSnapshot.render_relevant_change_between?(string_keyed, @template)
  end
  ...
  test 'legacy_previous_version? remains when prior_snapshot is nil' do
    ...
    @template.update_column(:version, 2)
