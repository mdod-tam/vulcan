...
module EmailTemplates
  ...
  class Audit
    ...
    # Matches test/mailers/text_email_link_accessibility_test.rb — staff-only, English seeds only.
    ...
    def run
      ...
      db_keys = EmailTemplate.pluck(:name, :locale, :format).map do |name, locale, format|
        [name, locale, EmailTemplate.formats.key(format)]
      end
    ...
    def expected_keys
      from_seeds = Dir.glob(SEED_DIR.join('*.rb')).flat_map { |path| parse_seed_file(path) }
      from_mailer = ACTION_TEMPLATE_ALIASES.values.uniq.map do |name|
        { name: name, locale: 'en', format: 'text' }
      end
