# frozen_string_literal: true

require 'uri'

WebAuthn.configure do |config|
  # Relying Party name
  config.rp_name = 'MAT Vulcan'

  application_host = ENV['APPLICATION_HOST'].to_s.strip.sub(%r{\Ahttps?://}i, '').split('/').first
  application_origin = nil
  application_uri = nil

  if application_host.present?
    application_origin = "https://#{application_host}"
    begin
      application_uri = URI.parse(application_origin)
    rescue URI::InvalidURIError
      raise ArgumentError, "APPLICATION_HOST must resolve to an absolute http/https origin: #{application_host.inspect}"
    end

    unless application_uri.is_a?(URI::HTTP) && application_uri.host.present?
      raise ArgumentError, 'APPLICATION_HOST must resolve to an absolute http/https origin'
    end
  end

  # Allowed origins (URLs where the app is accessed)
  allowed_origins = []
  allowed_origins << application_origin if application_origin.present?
  allowed_origins << 'http://localhost:3000' if Rails.env.development?

  if allowed_origins.empty? && Rails.env.production?
    raise ArgumentError, 'APPLICATION_HOST must be set to configure WebAuthn in production'
  end

  config.allowed_origins = allowed_origins.compact.uniq

  # Relying Party ID - explicitly set for localhost development.
  if Rails.env.development?
    config.rp_id = 'localhost'
  elsif ENV['WEBAUTHN_RP_ID'].present?
    config.rp_id = ENV['WEBAUTHN_RP_ID'].strip
  elsif application_uri
    config.rp_id = application_uri.host
  end

  # Optional: Configure timeout for credential creation/authentication
  config.credential_options_timeout = 120_000 # Milliseconds
end
